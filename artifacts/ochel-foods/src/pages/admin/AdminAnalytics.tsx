...
export default function AdminAnalytics() {
  ...
  const handleExport = () => {
    const csv = ["Date,Revenue (₦),Orders"]
      .concat(revenueData.map((r) => `"${r.date}",${r.revenue},${r.orders}`))
      .join("\n");
    ...
    <div className="space-y-6">
      ...
      {!loading && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Total Orders", value: orders.length.toString(), color: "text-blue-600", bg: "bg-blue-50" },
            { label: "Total Revenue", value: formatPrice(totalRevenue), color: "text-[#E8192C]", bg: "bg-red-50" },
            { label: "Delivered Revenue", value: formatPrice(deliveredRevenue), color: "text-green-600", bg: "bg-green-50" },
            { label: "Avg Order Value", value: formatPrice(avgOrderValue), color: "text-purple-600", bg: "bg-purple-50" },
          ].map((c) => (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={revenueData}>
                ...
                <XAxis dataKey="date" tick={{ fontSize: 10, fontFamily: "Montserrat" }}
